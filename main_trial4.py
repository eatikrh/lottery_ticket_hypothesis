...
def train_and_evaluate(width, base_epochs=75):
  model = Net(width)
  optimizer = optim.Adam(model.parameters(), lr=0.01)
  loss_fn = nn.MSELoss()

  # Dynamic training length: increase with width
...
def averaged_loss(width, trials=5):
  ...
  for i in range(trials):
    torch.manual_seed(i)
    train_loss, test_loss = train_and_evaluate(width)
...
plt.ylabel("MSE Loss")
plt.title("Double Descent in Neural Networks (Dynamic Epochs)")
plt.legend()
plt.grid()
